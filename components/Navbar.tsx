      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3 sm:gap-4 group">
          ...
          <div className="flex flex-col min-w-0">
            <span className="text-sm sm:text-xl font-serif font-bold tracking-tight text-text-main leading-none truncate pr-2">The Passing Wind</span>
        ...
        <div className="hidden lg:flex items-center gap-8 text-sm font-medium uppercase tracking-widest text-text-main">
          <Link href="#stories" onClick={() => setIsMenuOpen(false)} className="hover:text-brand transition-colors">{t.nav.stories}</Link>
          <Link href="#resources" onClick={() => setIsMenuOpen(false)} className="hover:text-brand transition-colors">{t.nav.resources}</Link>
          <Link href="#allies" onClick={() => setIsMenuOpen(false)} className="hover:text-brand transition-colors">{t.nav.allies}</Link>
        </div>
        ...
        <div className="lg:hidden absolute top-16 sm:top-20 left-0 w-full bg-bg-base border-b border-border-main p-6 space-y-6 shadow-xl animate-in slide-in-from-top duration-300">
          ...
          <div className="pt-6 border-t border-border-main flex flex-col gap-4">
            ...
            >
              ...
              {availableLanguages.find(l => l.code === language)?.label} → {availableLanguages.find(l => l.code !== language)?.label}
            </button>
          </div>
        </div>
      )}
